...
fn plan_select(
    ...
    let est_rows = DEFAULT_ROW_ESTIMATE;
    ...
    // Add join node
    if let Some(ref js) = join {
        let right_est = DEFAULT_ROW_ESTIMATE;
...
pub fn generate_plan(stmt: &Statement) -> Option<PlanNode> {
    match stmt {
        Statement::Select {
            ...
            limit,
            ..
        } => plan_select(from, items, join, filter, group_by, order_by, limit),
        _ => None,
    }
}
