...
use crate::facet::relational::{
    encode_schema_metadata, fts_index_meta_key, fts_index_prefix_for_table, index_meta_key,
...
use crate::sql::planner::{explain_plan, generate_plan, plan};
...
fn execute_stmt(db: &Database, session: &mut SqlSession, stmt: Statement) -> Result<SqlResult> {
    match stmt {
        ...
        Statement::Select {
            ...
            db, session, ctes, from, items, join, filter, as_of, valid_at, temporal, group_by,
...
fn execute_select(
    ...
    let (as_of, valid_at) = resolve_temporal_clauses(as_of, valid_at, &temporal)?;
    // Evaluate CTEs first
    ...
    let rows = if let Some(cte_rows) = cte_data.remove(&table_name) {
        ...
        // Apply WHERE filter (if not fully resolved by pk optimization or FTS)
        if let Some(ref f) = filter {
...
fn execute_ai_cluster_summary(db: &Database, filter: Option<&Expr>) -> Result<SqlResult> {
    ...
    let cluster_id = cluster_id.ok_or_else(|| {
        SpectraError::SqlExec("ai_cluster_summary requires WHERE cluster_id = '<id>'".to_string())
    })?;
...
fn execute_create_timeseries_table(
    ...
    let bucket_seconds = parse_interval_seconds(bucket_interval).ok_or_else(|| {
        SpectraError::SqlExec(format!("invalid bucket_size interval: '{bucket_interval}'"))
    })?;
...
fn update_ts_buckets(
    ...
    let ts_meta: TimeseriesMetadata = serde_json::from_slice(&ts_meta_bytes)
        .map_err(|e| SpectraError::SqlExec(format!("invalid timeseries metadata: {e}")))?;

    // Parse the doc as JSON to extract ts and value columns
    let json: serde_json::Value = serde_json::from_slice(doc)
        .map_err(|e| SpectraError::SqlExec(format!("invalid JSON for timeseries row: {e}")))?;
...
fn execute_explain_analyze(
    ...
    let plan_tree = generate_plan(&inner);
...
fn execute_explain(db: &Database, session: &mut SqlSession, inner: Statement) -> Result<SqlResult> {
    // Try cost-based plan tree first
    let plan_output = explain_plan(&inner);
    ...
    match inner {
        Statement::Select {
            ...
            if let TableRef::Named(ref table) = from {
                ...
                if is_doc_only && join.is_none() && filter.is_some() {
...
fn extract_pk_and_doc(
    ...
    if is_typed {
        ...
        let pk = val
            ...
            .ok_or_else(|| {
                SpectraError::SqlExec(format!("JSON row missing pk column '{}'", schema.pk))
            })?;
...
fn execute_table_function(name: &str, args: &[Expr]) -> Result<Vec<VisibleRow>> {
    ...
    match upper.as_str() {
        "READ_CSV" => {
            ...
            let content = std::fs::read_to_string(&path)
                .map_err(|e| SpectraError::SqlExec(format!("failed to read file '{path}': {e}")))?;
            let mut lines = content.lines();
            let header_line = lines
                .next()
                .ok_or_else(|| SpectraError::SqlExec("CSV file is empty".to_string()))?;
...
fn write_parquet(
    ...
    let file = std::fs::File::create(path)
        .map_err(|e| SpectraError::SqlExec(format!("failed to create file '{path}': {e}")))?;
    let mut writer = ArrowWriter::try_new(file, arrow_schema.clone(), None)
        .map_err(|e| SpectraError::SqlExec(format!("parquet writer error: {e}")))?;
    ...
    if is_typed {
        ...
        let batch = RecordBatch::try_new(arrow_schema, arrays)
            .map_err(|e| SpectraError::SqlExec(format!("arrow batch error: {e}")))?;
        writer
            .write(&batch)
            .map_err(|e| SpectraError::SqlExec(format!("parquet write error: {e}")))?;
    } else {
    ...
    writer
        .close()
        .map_err(|e| SpectraError::SqlExec(format!("parquet close error: {e}")))?;
    Ok(SqlResult::Affected {
        rows: rows.len() as u64,
        commit_ts: None,
        message: format!("exported {} rows to {}", rows.len(), path),
...
fn read_parquet_rows(path: &str) -> Result<Vec<VisibleRow>> {
    ...
    let file = std::fs::File::open(path)
        .map_err(|e| SpectraError::SqlExec(format!("failed to open file '{path}': {e}")))?;
    let builder = ParquetRecordBatchReaderBuilder::try_new(file)
        .map_err(|e| SpectraError::SqlExec(format!("parquet reader error: {e}")))?;
    let reader = builder
        .build()
        .map_err(|e| SpectraError::SqlExec(format!("parquet build error: {e}")))?;
    ...
    for batch_result in reader {
        let batch =
            batch_result.map_err(|e| SpectraError::SqlExec(format!("parquet read error: {e}")))?;
