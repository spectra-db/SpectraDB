...
impl UserManager {
    ...
    pub fn create_user(
        ...
        let value = serde_json::to_vec(&user)
            .map_err(|e| SpectraError::SqlExec(format!("failed to serialize user: {e}")))?;
        ...
        match db.get(key.as_bytes(), None, None)? {
            Some(bytes) => {
                let user: UserRecord = serde_json::from_slice(&bytes)
                    .map_err(|e| SpectraError::SqlExec(format!("failed to parse user: {e}")))?;
                if !user.enabled {
                    return Err(SpectraError::SqlExec(format!(
    ...
    pub fn change_password(db: &Database, username: &str, new_password: &str) -> Result<()> {
        ...
        match db.get(key.as_bytes(), None, None)? {
            ...
            None => Err(SpectraError::SqlExec(format!("user not found: {username}"))),
        }
    }
...
impl RoleManager {
    ...
    pub fn create_role(db: &Database, name: &str) -> Result<Role> {
        ...
        let value = serde_json::to_vec(&role)
            .map_err(|e| SpectraError::SqlExec(format!("failed to serialize role: {e}")))?;
        db.put(key.as_bytes(), value, 0, u64::MAX, None)?;
        Ok(role)
    }
        ...
        match db.get(key.as_bytes(), None, None)? {
            Some(bytes) => {
                let role: Role = serde_json::from_slice(&bytes)
                    .map_err(|e| SpectraError::SqlExec(format!("failed to parse role: {e}")))?;
                Ok(Some(role))
            }
            None => Ok(None),
        }
    ...
    pub fn grant_to_role(db: &Database, role_name: &str, permission: Permission) -> Result<()> {
        ...
        match db.get(key.as_bytes(), None, None)? {
            ...
            None => Err(SpectraError::SqlExec(format!(
