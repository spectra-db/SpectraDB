...
impl Default for ClusterConfig {
    fn default() -> Self {
        ClusterConfig {
            cluster_id: "spectradb-cluster".to_string(),
...
impl NodeRegistry {
    ...
    pub fn register_node(db: &Database, node: &NodeInfo) -> Result<()> {
        ...
        let value = serde_json::to_vec(node)
            .map_err(|e| SpectraError::SqlExec(format!("failed to serialize node: {e}")))?;
        ...
        match db.get(key.as_bytes(), None, None)? {
            Some(bytes) => {
                let node: NodeInfo = serde_json::from_slice(&bytes)
                    .map_err(|e| SpectraError::SqlExec(format!("failed to parse node: {e}")))?;
                Ok(Some(node))
            }
            None => Ok(None),
        }
        ...
        match db.get(CLUSTER_CONFIG_KEY.as_bytes(), None, None)? {
            Some(bytes) => {
                let config: ClusterConfig = serde_json::from_slice(&bytes).map_err(|e| {
                    SpectraError::SqlExec(format!("failed to parse cluster config: {e}"))
                })?;
                Ok(config)
            }
    ...
    pub fn save_cluster_config(db: &Database, config: &ClusterConfig) -> Result<()> {
        let value = serde_json::to_vec(config).map_err(|e| {
            SpectraError::SqlExec(format!("failed to serialize cluster config: {e}"))
        })?;
