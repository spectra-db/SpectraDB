...
pub fn create_event_store(db: &Database, name: &str, snapshot_interval: Option<u64>) -> Result<()> {
    ...
    let value = serde_json::to_vec(&meta)
        .map_err(|e| SpectraError::SqlExec(format!("failed to serialize event store: {e}")))?;
    ...
    match db.get(key.as_bytes(), None, None)? {
        Some(bytes) => {
            let meta: EventStoreMetadata = serde_json::from_slice(&bytes)
                .map_err(|e| SpectraError::SqlExec(format!("failed to parse event store: {e}")))?;
            Ok(Some(meta))
        }
        None => Ok(None),
    }
...
pub fn append_event(
    ...
    let event_value = serde_json::to_vec(&event)
        .map_err(|e| SpectraError::SqlExec(format!("failed to serialize event: {e}")))?;
...
pub fn save_snapshot(
    ...
    let snap_value = serde_json::to_vec(&snap)
        .map_err(|e| SpectraError::SqlExec(format!("failed to serialize snapshot: {e}")))?;
